...
| ルール | リスク |
|--------|--------|
| `terraform apply / destroy` | クラウドインフラの作成・変更・破壊を行う。 |
| `kubectl apply / delete` | Kubernetes クラスタ上のワークロードをデプロイまたは削除する。 |
| `helm install / upgrade` | Kubernetes パッケージのインストール・アップグレード。クラスタ全体に影響しうる。 |
| `docker push` | コンテナイメージをレジストリに公開する。 |
| `aws --no-cli-pager` | AWS CLI をページャなしで実行 — 破壊的な出力を見逃しやすい。 |
| `gcloud --quiet` | GCP CLI を確認プロンプトなしで実行する。 |
...
`psql`、`mysql`、`mongosh`、`sqlite3` などは破壊的な操作（`DROP TABLE`、`DELETE FROM`）が可能だが、Claude Code の deny ルールはコマンドの引数の中身までは区別できない。`Bash(psql *)` を deny にすると、分析に必要な `SELECT` クエリも含めてすべてブロックされてしまう。

**推奨:** データベースコマンドは deny にせず、デフォルト（都度確認）のままにする。プロンプトが出たときに内容を確認するのが、読み取りクエリと破壊的操作を区別する唯一の方法。
