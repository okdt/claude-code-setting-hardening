...
このチートシートは `~/.claude/settings.json` を通じて Claude Code 環境をハードニング（堅牢化）するための実践ガイドです。**サンドボックスを主たる防御**とし、パーミッション・フックを補助として、最小権限の原則や Human In the Loop（HITL）をどのように実践するかを考えるのに良い題材でしょう。

### 基本的なアプローチ — 三層防御モデル

他のコーディングエージェント（例えば [Codex CLI](https://github.com/okdt/codex-cli-hardening-cheatsheet)）と同じレイヤリング原則に従います：最も強力でバイパスが困難な制御から始め、より細かい層は必要な場合にのみ追加します。

| レイヤー | メカニズム | 役割 | AIがバイパス可能？ |
|---------|-----------|------|:----------------:|
| **1. サンドボックス** | OS レベルのプロセス隔離 | カーネルレベルでファイル・ネットワークアクセスを制限。これが基盤 — 他のすべてのレイヤーが失敗しても、サンドボックスが被害を封じ込めます。 | 不可 |
| **2. パーミッション** | allow / deny / ask ルール | ツール呼び出し時の動作を制御：常に許可、常に拒否、毎回確認、デフォルト。サンドボックスの*上に*きめ細かいコマンド単位の制御を追加。`ask` で Human-in-the-Loop を実現。 | 不可（ハーネスが強制） |
| **3. フック** | PreToolUse シェルスクリプト | ツール呼び出し前にカスタムロジックを実行。glob ベースの deny ルールでは安全と危険を区別できないケース（例：`SELECT` vs `DROP TABLE`）に対応。 | 不可（ハーネスが強制） |

第4のレイヤー — **ログ**（OpenTelemetry）— はエンタープライズ環境やデバッグで必要になるかもしれません。簡単に触れています。

> **ポイント:** これら複数の防御の手立てを仕込んでおくアプローチを **多層防御** といいます。重要な考え方は、サンドボックスに重い仕事をさせること。パーミッションとフックは代替ではなく*補助*です。
...
サンドボックスが OS レベルで被害を防ぐのが主たる防御です。Deny ルールは、サンドボックスでは防げないケース（例：CWD 内での破壊的 git 操作）に対する第二の層、またはサンドボックスが既に対処しているケースに対する多層防御として機能します。Hooks は環境固有のカスタムロジックを追加できます。

ここで、何をブロックし、何を許可し、何を常に確認すべきか、そして deny ルールだけでは不十分な場合にどうすべきかなどを解説します。なお、本ドキュメントの deny リストはサンプルであり、コンプリートリストではありません。各ルールには **必須**（サンドボックスでは防げない）または **多層防御**（サンドボックスが主たる防御）のタグが付けられています。主に macOS 環境で執筆・検証していますが、Linux や Windows（WSL）でも参考になるはずです。
...
### サンドボックスが防げること・防げないこと

サンドボックスが*何を防げて*、*何を防げないか*を理解することが、deny ルールやフックにどこまで投資すべきかの判断に不可欠です。サンドボックスが OS レベルで既にブロックしている脅威に対する deny ルールは多層防御——あれば良いが、主たる防御ではありません。サンドボックスが*防げない*脅威に注力しましょう。

| 脅威 | サンドボックスで防止？ | 理由 |
|------|:-------------------:|------|
| CWD 外のファイルアクセス（`rm -rf /`、`~/.ssh` の読み取り） | **はい** | ファイルシステムが CWD と許可パスに制限される |
| ネットワークアクセス（`ssh`、`curl`、`wget`） | **はい** | サンドボックス有効時、OS レベルでネットワークがブロックされる |
| 破壊的 git 操作（`push -f`、`reset --hard`） | **いいえ** | git は CWD 内の `.git/` を操作する——サンドボックスはこれを許可する |
| 広範なステージング（`git add .`、`git add -A`） | **いいえ** | CWD 内で動作する |
| パッケージ公開（`npm publish`） | **部分的** | ネットワークがオフならブロック。しかしネットワーク有効時（例：`npm install` のため）は publish も通る |
| インフラコマンド（`terraform apply`、`kubectl`） | **部分的** | 認証情報・設定がアクセス可能なパス内にあるかによる |
| MCP アクション（Slack 送信、カレンダー書き込み） | **いいえ** | MCP ツールは Claude のツールシステムを経由し、OS レベルのネットワークを通らない |
| macOS 固有（`open`、`osascript`） | **いいえ** | サンドボックス境界外でプロセスを起動する |
| データベースコマンド（`psql`、`mysql`） | **部分的** | DB ソケット/ポートへのアクセス可否による。サンドボックスは `SELECT` と `DROP TABLE` を区別できない |

**実践的な結論:** セクション 4.1（破壊的 git）、4.7（公開/デプロイ）、4.8（macOS）、4.9（インフラ）、4.11（MCP）の deny ルールは**必須**です——サンドボックスだけではこれらの脅威を防げません。その他の deny ルール（4.2 ファイル操作、4.3 システム操作、4.5 リモートコード実行、4.6 リモートアクセス）は**多層防御**です——サンドボックスが主たる防御で、deny ルールは第二の層を追加します。
...
> **このセクションの読み方:** **必須** タグが付いたルールは、サンドボックスでは防げない脅威に対するものです——これらを優先してください。**多層防御** タグが付いたルールは、サンドボックスが既に OS レベルでブロックしている脅威に対するもので、deny ルールは第二の層を追加しますが、唯一の防御線ではありません。ゼロから始めるなら、まず必須ルールを実装してください。

### 4.1 Deny — 破壊的な Git 操作 <sup>必須</sup>
...
### 4.2 Deny — 破壊的ファイル操作 <sup>多層防御</sup>
...
### 4.3 Deny — 危険なシステム操作 <sup>多層防御</sup>
...
### 4.5 Deny — パイプ経由のリモートコード実行 <sup>多層防御</sup>
...
### 4.6 Deny — リモートアクセス <sup>多層防御</sup>

Claude Code がリモートホストへ接続することを防ぎます。AIアシスタントにリモート接続を許可すべきではありません。これらのコマンドはリモートホストへのファイル転送やコマンド実行が可能です。
サンドボックスモードが有効な場合、ネットワークアクセスは OS レベルでブロックされます——サンドボックスがここでの主たる防御です。これらの deny ルールは、サンドボックスの設定ミスや一時的な無効化に備えた第二の層として機能します。
...
### 4.7 Deny — パッケージ公開とデプロイ <sup>必須</sup>
...
### 4.8 Deny — つい許可しがちだが取り返しがつかないもの (macOS) <sup>必須</sup>
...
### 4.9 Deny — インフラストラクチャ <sup>必須</sup>
...
### 4.10 Deny — 機密ファイルへのアクセス <sup>多層防御</sup>
...
### 4.11 Deny — MCP アクションによるなりすましメッセージの防止 <sup>必須</sup>
